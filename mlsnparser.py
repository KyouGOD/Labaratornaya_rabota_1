...
def parser(url):
    try:
        ...
        for link in addresspars:
            ...
            try:
                data['addresses'].append(link.contents[1])  # находит адрес
            except IndexError:
                data['addresses'].append('Не указана')  # не нашел адрес
        ...
        for price in pricepars:
            ...
            data['prices'].append(price.find('span', class_='main-param').text.replace('\xa0', ''))  # находит арендную
            # плату за 1 месяц

        i = 0
        for floor in floorpars:
            a = floor
            i = commentdef(a, i, data)
        ...
        for square in squarepars:
            ...
            if square.find('span', class_='main-param') is None:  # обрабатывает объявления, где не указана площадь
                ...
                continue
            data['square'].append(square.find('span', class_='main-param').text.replace('\xa0', ''))  # находит площадь
            # жилья
